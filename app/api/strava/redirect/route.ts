...
export async function GET(request: Request) {
  ...
  const dashboardOrigin = getSiteUrl();

  if (errorParam) {
    const redirectUrl = buildDashboardRedirect(dashboardOrigin, "error", errorParam);
    return redirectWithClearedState(redirectUrl);
  }

  if (!code || !state || !storedState || state !== storedState) {
    const redirectUrl = buildDashboardRedirect(dashboardOrigin, "error", "Invalid state or missing code");
    return redirectWithClearedState(redirectUrl);
  }
  ...
  if (!user) {
    const redirectUrl = buildDashboardRedirect(dashboardOrigin, "error", "Sign in to connect Strava");
    return redirectWithClearedState(redirectUrl);
  }
  ...
  try {
    ...
    if (error) {
      const redirectUrl = buildDashboardRedirect(dashboardOrigin, "error", error.message);
      return redirectWithClearedState(redirectUrl);
    }

    const redirectUrl = buildDashboardRedirect(dashboardOrigin, "connected");
    return redirectWithClearedState(redirectUrl);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to connect to Strava";
    const redirectUrl = buildDashboardRedirect(dashboardOrigin, "error", message);
    return redirectWithClearedState(redirectUrl);
  }
}
