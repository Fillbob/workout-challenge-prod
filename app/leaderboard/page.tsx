...
export default function LeaderboardPage() {
  ...
  const focusedEntry = useMemo(() => rows.find((row) => row.user_id === focusedUser), [focusedUser, rows]);
                ...
                <div className="rounded-3xl border border-orange-100 bg-white/80 p-5 shadow-lg shadow-orange-100/70 backdrop-blur">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="space-y-1 text-right text-xs text-slate-500">
                      <p>Click an icon to spotlight their lane.</p>
                      {focusedEntry && (
                        <div className="inline-flex items-center gap-2 rounded-full border border-orange-100 bg-orange-50 px-3 py-1 text-[11px] font-semibold text-orange-700 shadow-sm shadow-orange-100">
                          <ProfileCircle iconId={focusedEntry.icon} name={focusedEntry.name} size="sm" />
                          <span className="text-slate-700">{focusedEntry.name}</span>
                        </div>
                      )}
                    </div>
                  </div>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <button
                              ...
                              title={isFocused ? `Spotlighting ${row.name}` : `Spotlight ${row.name}`}
                            >
